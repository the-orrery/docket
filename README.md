...
| `docket resolve <ref> [--json]` | 把 uid、当前 display ref、历史 alias、legacy id 解析成同一个 issue；`--json` 是其他工具的机器契约。 |
| `docket migrate-identity [--dry-run]` | 给旧 PM 数据补齐 identity v3 字段：`uid`、项目内 `project_iid`、历史 `aliases`。 |
| `docket start <id>` | 标记为进行中。 |
| `docket finish <id>` | 标记为完成；关闭前会检查该 issue 是否还有 active registrar worktree。 |
...
| 字段/引用 | 用途 |
|---|---|
| `uid: dkt_<32 hex>` | 机器主键，永不改，registry/artifact/自动化优先保存它。 |
| `project_iid` + project prefix | 人类 display ref，例如 `WORK-12`；编号在项目内递增。 |
| `id` / 文件名 | legacy storage id，例如 `ISSUE-286` / `ERI-905`，继续用于文件名、评论文件和旧历史兼容。 |
| `aliases` | 历史引用集合；旧 display ref、legacy id、迁移前后的引用都不能复用给别的 issue。 |

`<id>` 参数实际是 `<ref>`：支持 `dkt_...`、当前 display ref、历史 alias、legacy id
和在不歧义时的裸数字。裸数字如果同时命中多个项目内编号会报 ambiguous，不会猜。
其他工具不要自己 parse `WORK-12`，统一调用：

```sh
docket resolve WORK-12 --json
```

`resolve` 先查当前 PM root；当前 root 查不到时，会读取
`~/.config/docket/tiers.toml` 的 configured tiers 继续查找。只有单 tier 命中才会返回；
跨 tier 多命中会报 ambiguous。JSON 输出包含 `root` 和 `tier`，供 registrar 等工具确认
真实归属。

新 issue 会自动写入 `uid`、`project_iid` 和 `aliases`。旧数据执行一次：

```sh
docket migrate-identity --dry-run
docket migrate-identity
```
...
自身的 Git 历史。artifact repo 根目录会写 `.docket-artifact.yaml`，记录
`issue_uid`、legacy `issue_id`、当前 `display_ref` 和 aliases；PM issue 正文只保留状态、
决策和链接。
